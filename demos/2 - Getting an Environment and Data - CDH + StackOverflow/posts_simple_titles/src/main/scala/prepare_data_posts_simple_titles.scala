/**
 * Loads Posts, only titles, from XML into HDFS
 *
 ...
 * Or run using
 *   sbt package
 *   spark2-submit --class "PreparePostsSimpleTitlesApp" target/scala-2.11/posts-simple-titles-project_2.11-1.0.jar
 * 
...
object PreparePostsSimpleTitlesApp {
  def main(args: Array[String]) {
    val spark = SparkSession.builder.appName("PreparePostsTitlesCSV").getOrCreate() 
    ...
    postFields.saveAsTextFile("/user/cloudera/stackexchange/simple_titles_txt")
  }

// extract the values in each xml row
def createRecord(nd : scala.xml.Elem) = {  
  val title = (nd \ "@Title").toString.replace("?", "")
  (title)
  }
}
